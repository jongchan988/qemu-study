...
//SystemRegisters
static TCGv cpu_sysr[JC1904A_SYS_REG];
...
void jc1904_tcg_init(void){
    ...
    for(i = 0;i < NUM_REG_PAGE_SIZE; ++i) {
        cpu_r[i] = tcg_global_mem_new_i32(cpu_env,
                                          offsetof(CPUJC1904AState, r[i]),
                                          jc1904_cpu_r_names[i]);
    }

    for(i = 0;i < JC1904A_SYS_REG; ++i) {
        char* name;
        ...
        cpu_sysr[i] = tcg_global_mem_new_i32(cpu_env,
                                             offsetof(CPUJC1904AState, sysr[i]),
                                             name);
        free(name);
    }
    ...
    for(i = 0;i < 32; ++i) {
        cpu_sflags[i] = tcg_global_mem_new_i32(cpu_env,
                                               offsetof(CPUJC1904AState, sflags[i]),
                                               jc1904_cpu_sr_flag_names[i]);
    }
}
...
static bool trans_ACALL(DisasContext *ctx, arg_ACALL *a){
    ...
    tcg_gen_addi_i32(cpu_r[JC1904A_LR_REG], cpu_r[JC1904A_LR_REG], 0x2);
    tcg_gen_add_i32(cpu_r[JC1904A_PC_REG], temp, cpu_sysr[2]);
...
static bool trans_RCALL_f1(DisasContext *ctx, arg_RCALL_f1 *a){
    ...
    tcg_gen_addi_i32(cpu_r[JC1904A_LR_REG], cpu_r[JC1904A_PC_REG], 2);
    tcg_gen_addi_i32(cpu_r[JC1904A_PC_REG], cpu_r[JC1904A_PC_REG], disp);
...
static void jc1904_tr_init_disas_context(DisasContextBase *dcbase, CPUState *cs){
    CPUJC1904AState *env = cs->env_ptr;
...
static void jc1904_tr_tb_start(DisasContextBase *db, CPUState *cs){

}

static void jc1904_tr_insn_start(DisasContextBase *dcbase, CPUState *cs){
    DisasContext *ctx = container_of(dcbase, DisasContext, base);
    tcg_gen_insn_start(ctx->base.pc_next);
}

static void jc1904_tr_translate_insn(DisasContextBase *dcbase, CPUState *cs){
    DisasContext *ctx = container_of(dcbase, DisasContext, base);
    uint32_t insn;
    ...
    if (!decode_insn(ctx, insn)) {
        error_report("[JC1904-TCG] jc1904_tr_translate_insn, illegal instr, pc: 0x%04x\n", ctx->base.pc_next);
        gen_helper_raise_illegal_instruction(cpu_env);
    }
}

static void jc1904_tr_tb_stop(DisasContextBase *dcbase, CPUState *cs){
    DisasContext* ctx = container_of(dcbase, DisasContext, base);
    switch(ctx->base.is_jmp) {
        ...
        case DISAS_EXIT:
            ...
            break;
        default:
            printf("[jc1904_tr_tb_stop] ERROR: undefined condition\n");
            g_assert_not_reached();
    }
}

static void jc1904_tr_disas_log(const DisasContextBase *dcbase,
...
static const TranslatorOps jc1904_tr_ops = {
        .init_disas_context = jc1904_tr_init_disas_context,
        .tb_start           = jc1904_tr_tb_start,
        .insn_start         = jc1904_tr_insn_start,
        .translate_insn     = jc1904_tr_translate_insn,
        .tb_stop            = jc1904_tr_tb_stop,
        .disas_log          = jc1904_tr_disas_log,
};
...
{
    DisasContext dc = { };
    translator_loop(cs, tb, max_insns, pc, host_pc, &jc1904_tr_ops, &dc.base);
}